...
export const get_order_stats = async (req: Request, res: Response) => {
  try {
    ...
    if (preset) {
      // Handle "all" preset - no date filtering
      if (preset === "all") {
        // No date filtering for "all" - leave startDate and endDate undefined
      } else {
        const now = new Date();
        const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        const endOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59, 999);
        ...
        switch (preset) {
          ...
          default:
            // Unknown preset - default to period-based filtering
    ...
    // Use baseFilter for revenue calculations
    const revenueFilter = { ...baseFilter, status: OrderStatus.DELIVERED, paymentStatus: "paid" };

    // Use baseFilter for order value filter
    const orderValueFilter = { ...baseFilter };
    ...
    const [
      ...
      order_model.countDocuments(baseFilter),
      order_model.countDocuments({ ...baseFilter, status: OrderStatus.PENDING }),
      order_model.countDocuments({ ...baseFilter, status: OrderStatus.PROCESSING }),
      order_model.countDocuments({ ...baseFilter, status: OrderStatus.DELIVERED }),
      order_model.countDocuments({ ...baseFilter, status: OrderStatus.CANCELLED }),
      order_model.aggregate([
        { $match: revenueFilter },
        { $group: { _id: null, total: { $sum: "$totalAmount" } } },
      ]),
      order_model
        .find(orderValueFilter)
        .sort({ orderDate: -1 })
        .limit(10)
    ...
    // Process trends data to fill missing days based on date range
    const trendsData: any[] = [];
    const revenueData: any[] = [];

    // Determine the date range for trends
    let trendStartDate: Date;
    let trendEndDate: Date;

    if (preset === "all") {
      // For "all time", show last 30 days as default trend period
      trendEndDate = new Date();
      trendStartDate = new Date();
      trendStartDate.setDate(trendStartDate.getDate() - 29);
    } else if (startDate && endDate) {
      // Use the actual filter date range
...
export const get_delivery_stats = async (req: Request, res: Response) => {
  try {
    ...
    // Build date filter (same logic as orders stats)
    let startDate: Date | undefined;
    let endDate: Date | undefined;
    ...
    // Process delivery trends data to fill missing days dynamically based on requested range
    const deliveryTrendsData: any[] = [];

    // Determine trend range similar to orders trends logic
    let trendStartDate: Date;
    let trendEndDate: Date;

    if (preset === "all") {
      // For "all time", default to last 30 days for trend visualization
      trendEndDate = new Date();
      trendStartDate = new Date();
      trendStartDate.setDate(trendStartDate.getDate() - 29);
    } else if (startDate || endDate) {
      // Use the explicitly resolved start/end
      trendStartDate = startDate ? new Date(startDate) : new Date();
      trendEndDate = endDate ? new Date(endDate) : new Date();
    } else {
      // Fall back to `period` days (defaults handled earlier)
    ...
    const iterStart = new Date(trendStartDate.getFullYear(), trendStartDate.getMonth(), trendStartDate.getDate());
    const iterEnd = new Date(trendEndDate.getFullYear(), trendEndDate.getMonth(), trendEndDate.getDate());

    for (let dt = new Date(iterStart); dt <= iterEnd; dt.setDate(dt.getDate() + 1)) {
